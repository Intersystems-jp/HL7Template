...
テンプレートクラスにHL7メッセージを記載し、必要な部分に#(..プロパティ)#を指定することで、プロパティ値からHL7メッセージに変換し、出力します。

## インストール方法

src配下のソースコードを読み込みます。
HL7Templateは、HL7 v2.xのスキーマが必要です。
SS-MIX2のスキーマを使用する場合は、以下の手順でSS-MIX2.HL7を読み込んでください。

1. 管理ポータルの「Interoperability」「相互運用」「HL7 v2.x」「HL7 v2.x スキーマ構造」メニューを選択
2. 「インポート」ボタンをクリック
3. リポジトリ上のSS-MIX2.HL7を選択し、「OK」ボタンをクリック
